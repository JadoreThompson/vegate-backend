...
from service.ohlc.loader.logging.record import OHLCLoadCompleteRecord, OHLCLoadStartRecord, OHLCLogRecord
...
class AlpacaOHLCLoader(BaseOHLCLoader):
    ...
    def _get_walogger(self, symbol: str, timeframe: Timeframe) -> WALogger:
        if self._walogger is None:
            self._walogger = WALogger(BrokerType.ALPACA, symbol, timeframe)
    ...
    async def load_candles(
        ...
        async for bar_batch, fetch_params in self._fetch_bars(
            symbol, timeframe, start_date, end_date, last_record.params if last_record is not None else None
        ):
    ...
    async def _fetch_bars(
        ...
        params: dict | None = None,# Params from last run
    ):
        if params is None and (symbol is None or timeframe is None or start_date is None or end_date is None):
            raise ValueError("Either params or all of symbol, timeframe, start_date, end_date must be provided")
